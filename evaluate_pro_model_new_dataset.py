"""
Pro Model Evaluation with New Dataset (job_applicant_dataset_new.csv)
Tests with first 5000 records from the new 20,000-record dataset
...
print("=" * 90)
print("🚀 PRO MODEL EVALUATION - NEW DATASET (job_applicant_dataset_new.csv)")
print("=" * 90)
...
try:
    df_new = pd.read_csv('job_applicant_dataset_new.csv')
    print(f"✅ Total records in new dataset: {len(df_new)}")
except Exception as e:
    print(f"❌ Error loading dataset: {e}")
    exit(1)

# Use first 5000 records (from first 10K as requested)
df = df_new.head(5000).copy()
print(f"✅ Using first 5,000 records from new dataset")
...
print(f"\n📈 Class Distribution (New Dataset):")
...
test_size = min(2000, int(len(df) * 0.4))
...
if best_predictions is None:
    predictions = (similarities >= 0.48).astype(int)
    best_threshold = 0.48
else:
...
print("\n" + "=" * 90)
print("✨ PRO MODEL EVALUATION RESULTS (NEW DATASET)")
print("=" * 90)
...
output_dir = 'models/pro_model_evaluation_new_dataset'
os.makedirs(output_dir, exist_ok=True)

metrics = {
    'model_type': model_type,
    'dataset': 'job_applicant_dataset_new.csv (first 5000 records)',
...
try:
    ...
    if fpr is not None and tpr is not None:
        ...
        plt.title('ROC Curve - Pro Model (New Dataset)', fontsize=14, fontweight='bold')
    ...
    plt.title('Similarity Distribution by Class (New Dataset)', fontsize=14, fontweight='bold')
    ...
    ax.set_title('Pro Model - Key Performance Metrics (New Dataset)', fontsize=14, fontweight='bold')
    ...
    if len(df_results) > 0:
        ...
        ax.set_ylabel('Score', fontsize=12)
        ax.set_title('Pro Model Performance Across Thresholds (New Dataset)', fontsize=14, fontweight='bold')
...
print(f"\n✨ KEY FINDINGS:")
print(f"   • Dataset: job_applicant_dataset_new.csv (first 5000 records)")
print(f"   • Test Set Size: {len(test_df)} samples")
