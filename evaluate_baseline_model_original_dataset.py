"""
Baseline Model Evaluation with Original Dataset (job_applicant_dataset.csv)
Tests the default model (all-MiniLM-L6-v2) with original 10,000-record dataset
...
print("=" * 90)
print("🚀 BASELINE MODEL EVALUATION - ORIGINAL DATASET (job_applicant_dataset.csv)")
print("=" * 90)
...
# Use 25% for testing (similar to pro model evaluation)
...
print("\n" + "=" * 90)
print("🤖 STEP 3: Loading Baseline Model")
print("=" * 90)

model_load_start = time.time()
try:
    model = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')
    print(f"✅ Baseline model loaded: all-MiniLM-L6-v2")
    model_type = "Baseline Model (all-MiniLM-L6-v2)"
except Exception as e:
...
print("\n" + "=" * 90)
print("✨ BASELINE MODEL EVALUATION RESULTS (ORIGINAL DATASET)")
print("=" * 90)
...
output_dir = 'models/baseline_model_evaluation_original_dataset'
...
try:
    ...
    plt.title(f'Confusion Matrix - Baseline Model (Original Dataset)\nThreshold: {best_threshold:.2f}', fontsize=14, fontweight='bold')
    ...
    if fpr is not None and tpr is not None:
        ...
        plt.title('ROC Curve - Baseline Model (Original Dataset)', fontsize=14, fontweight='bold')
    ...
    plt.title('Similarity Distribution by Class (Original Dataset - Baseline Model)', fontsize=14, fontweight='bold')
    ...
    ax.set_title('Baseline Model - Key Performance Metrics (Original Dataset)', fontsize=14, fontweight='bold')
    ...
    if len(df_results) > 0:
        ...
        ax.set_ylabel('Score', fontsize=12)
        ax.set_title('Baseline Model Performance Across Thresholds (Original Dataset)', fontsize=14, fontweight='bold')
...
print(f"   • Model: Baseline (all-MiniLM-L6-v2)")
print(f"   • Test Set Size: {len(test_df)} samples")
